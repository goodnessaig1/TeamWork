const getAllFeeds = `
SELECT a.article as post, a.title as title,a.created_at as post_date, a.article_id as postId, c.comment as comment, c.created_at as date, CONCAT(u.first_name, ' ', u.last_name) as post_author,u2.profile_pix as comment_author_profile, u2.first_name as comment_author,u2.last_name as comment_author_last_name,u.jobrole as author_jobrole, u.profile_pix,
...
EXISTS(SELECT * FROM gif_likes l WHERE l.gif_id = g.gif_id and l.author_id = $1) AS isLiked
FROM gifs g
LEFT JOIN gif_comment c ON c.gif_id = g.gif_id
LEFT JOIN users u ON u.id = g.user_id
LEFT JOIN users u2 ON u2.id = c.author_id
ORDER BY post_date DESC, date
`;
